        ...
        <Table className="bg-slate-100 rounded-xl shadow-lg w-[95%] xs:w-[500px] sm:w-[600px] md:w-[700px]">
            ...
            <TableBody>
                {books.length > 0 ? books.map((book) => {
                    return (
                        <BookTableRow {...book} key={book._id} />
                    )
                }):
                <div className="py-8  flex items-center justify-center gap-4">
                    ...
                    <Link to={'/'} className="border-b border-black hover:text-green-800 hover:border-green transition-all duration-150">
                        go back
                    </Link>
                </div>
                }
...
function BookTableRow({
    title, authors, yearPublished, dateAdded
}: Book) {
    return (
        <TableRow className="hover:bg-slate-300" >
            ...
            <TableCell >{new Date(dateAdded).toDateString().split(' ').splice(1).join(' ')}</TableCell>
        </TableRow>
    )   
}